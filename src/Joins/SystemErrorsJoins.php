...
namespace BlackCat\Database\Packages\SystemErrors\Joins;
...
final class SystemErrorsJoins {
    ...
    /**
     * FK: system_errors -> users
     * LEFT JOIN vw_users AS $as ON $as.id = $alias.resolved_by
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinUsersByResolvedBy(string $alias = 't', string $as = 'j1'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_users AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.resolved_by' . ' ', []];
    }
