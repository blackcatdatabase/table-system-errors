 ...
 */
final class SystemErrorsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private SystemErrorRepository $systemErrorRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'system_errors' => $this->systemErrorRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\SystemErrors\ModuleException("Record {$idStr} not found for locking.");
            }
