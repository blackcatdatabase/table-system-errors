...
namespace BlackCat\Database\Packages\SystemErrors\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\SystemErrors\Definitions;
use BlackCat\Database\Packages\SystemErrors\Criteria;
use BlackCat\Database\Packages\SystemErrors\Dto\SystemErrorDto as Dto;
use BlackCat\Database\Packages\SystemErrors\Mapper\SystemErrorDtoMapper as RowMapper;
...
final class SystemErrorRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\SystemErrors\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'fingerprint' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\SystemErrors\Definitions::table());
            return $bulk->upsertMany($rows, [ 'fingerprint' ], [ 'level', 'message', 'exception_class', 'file', 'line', 'stack_trace', 'token', 'context', 'occurrences', 'user_id', 'ip_hash', 'ip_hash_key_version', 'ip_text', 'ip_bin', 'user_agent', 'url', 'method', 'http_status', 'resolved', 'resolved_by', 'resolved_at', 'last_seen' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\SystemErrors\Dto\SystemErrorDto|null */
    public function getByFingerprint(string $fingerprint, bool $asDto = false): array|\BlackCat\Database\Packages\SystemErrors\Dto\SystemErrorDto|null {
        $row = $this->getByUnique([ 'fingerprint' => $fingerprint ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\SystemErrors\Mapper\SystemErrorDtoMapper::fromRow($row);
    }
    public function existsByFingerprint(string $fingerprint): bool {
        $where = 't.' . Ident::q($this->db, 'fingerprint') . ' = :uniq_fingerprint';
        return $this->exists($where, [ 'uniq_fingerprint' => $fingerprint ]);
    }
    /** @return int|string|null */
    public function getIdByFingerprint(string $fingerprint) {
        $row = $this->getByFingerprint($fingerprint);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\SystemErrors\Dto\SystemErrorDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\SystemErrors\Dto\SystemErrorDto|null {
