...
use BlackCat\Database\Packages\SystemErrors\Repository\SystemErrorRepositoryInterface;
...
class SystemErrorRepository implements SystemErrorRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'fingerprint' ];
    ...
    public function getByFingerprint(string $fingerprint, bool $asDto = false): array|\BlackCat\Database\Packages\SystemErrors\Dto\SystemErrorDto|null {
        return $this->getByUnique([ 'fingerprint' => $fingerprint ], $asDto);
    }
    ...
    public function getIdByFingerprint(string $fingerprint) {
        $row = $this->getByFingerprint($fingerprint, false);
